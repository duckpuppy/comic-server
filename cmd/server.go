...
func runServer(cmd *cobra.Command, args []string) error {
	...
	if cfg.Server.Komga.Enabled {
		...
		komgaSyncer, err = buildKomgaSyncer(cfg.Server.Komga, cfg.Server.LibrarySourceRoot, cfg.Server.LibraryMountRoot, configDB, backend)
		if err != nil {
			return err
		}
...
func buildKomgaSyncer(cfg config.KomgaConfig, librarySourceRoot, libraryMountRoot string, configDB *configdb.DB, backend library.Backend) (*komga.Syncer, error) {
	...
	return komga.NewSyncer(backend, komga.SyncOptions{
		...
		LibrarySourceRoot: librarySourceRoot,
		LibraryMountRoot:  libraryMountRoot,
		Targets:           targets,
